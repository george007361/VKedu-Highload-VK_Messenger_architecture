...
### Сводная таблица RPS по типовым действиям <a name="rps-table"></a>
|   №   | Действие                             |  Тип   | RPS     |
| :---: | ------------------------------------ | :----: | :------ |
...
## Физическая схема базы данных <a name="db-phys"></a>
...
## Список серверов
**Исходные данные:**
1. [Cводная таблица размера элемента в хранилище по типам](#stor-elem-size)
2. [RPS по типовым запросам](#rps-table)
3. Коэффициент нагрузки: **2**
...
Необходимое количество ядер: **372**
> 744 270 / 2 000 = 372 
...
Необходимое количество инстансов умещается на одной машине. Предусмотрим отказ оборудования и разместим БД на двух машинах
...
Итого необходиомо поддержать: **566 842 RPS** с учетом пиковых нагрузок.
...
Увеличиваем количество серверов до 10 и аналогичными расчётами получаем нагрузку 60% на сеть и 75% на CPU в случае падения машины.  
