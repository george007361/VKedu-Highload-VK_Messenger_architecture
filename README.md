...
3. Создание чатов в день: *0,06 раз*
...
4. Удаление чатов в сутки: предположим, что оно *пренебрежительно мало*
5. Отправка новых сообщений в день: *272 текстовых и 26 голосовых сообщения*
...
6. Изменение отправленных сообщений в день: *28 сообщения*
...
7. Удаление отправленных сообщений в день: *3 сообщения*
...
8. Поиск пользователей за день: *0,06 раз*
...
9. Получение списка диалогов: *15 раз*
...
10.  Загрузка новых вложений в день: *6*
...
11. Получение вложений из сообщений в день: *15 штук*
    > Из п.9 считаем, что 2% сообщений имеют вложения, из п.8 имеем, что в день пользователь заходит в сервис и получает список диалогов 15 раз. Будем считать, что в среднем пользователь имеет 5 активных диалогов, в которых каждый раз проверяет последние сообщения, количество которых примем равным 10.
   
    > Тогда в день пользователь получает вложений 15 * 5 * 10 * 2% = 15

    **RPS на получение вложений**: *8576*
    > 15 * 49,4 * 1000000 / (24 * 60 * 60) = 8576
12. Получение списка сообщений в диалоге: *80 раз*
...
>  * 100 * 10^6 = 211600000000 байт = 0,19 Тб

#### Размер хранения по типам данных
1. Текстовые сообщения: *47 723 293,3 Тб*
   > Используем п.4 продуктовых требований, откуда 0,914 * 15 млрд * 1000  = 13,71 млрд текстовых сообщений в день отправляют все пользователи. 

   > Рассчитаем количество сообщений за 10 лет, пренебрегая ростом аудитории и считая статистику отправки постоянной.

   > Получится 13,71 млрд * 365 * 10 = 50041,5 млрд сообщений за 10 лет.

   > Принимая максимальную длину сообщений в 1024 символа (1 мб), получится, что хранилище сообщений должно вмещать 
   50041,5 млрд * 1 мб = 47 723 293,3 Тб
2. Голосовые сообщения: *125 730 514,53 Тб*
   > Расчёты аналогичны предыдущему пункту

   > 0,086 * 15 млрд * 1000 = 1,29 млрд сообщений в день

   > Расчёт выполняем за 10 лет

   > Получаем: 1,29 млрд * 365 * 10 = 4708,5 млрд сообщений за 10 лет

   > Принимаем среднюю длину голосового сообщения за 15 секунд, кодирование OGG (192 кбит / с), откуда вес одного ГС 2880 кб или 2,8 мб

   > Итого 4708,5 млрд * 2,8 мб = 125 730 514,53 Тб
3. Вложения в сообщениях: *1 908 931,73 Тб *
   > Из предположения п.9 продуктовых требований, 2% текстовых сообщений имеют вложения. Вложениями считаем фотографии средним весом 2 мб.

   > Расчёт выполняем за 10 лет

   > Из п.1 технических расчетов имеем 50041,5 млрд сообщений за 10 лет.

   > Тогда объем хранилища вложений составит 50041,5 млрд * 2% * 2мб = 2001,66 млрд мб = 1 908 931,73 Тб 
#### Сетевой трафик
...
3. Работа с текстовыми сообщениями:
   ...
   Создание: *1269,44 Гбит/с*
   > 1 мб * 158680 RPS = 1269,44 Гбит/с

   Изменение: *128,072 Гбит/с*
   > 1 мб * 16009 RPS = 128,072 Гбит/с

   Получение: *327,7 Гбит/с*
   > 1 мб * 98% * 0,914 * 45741 RPS = 327,7 Гбит/с
4. Работа с голосовыми сообщениями: 

   Создание: *334,432 Гбит/с*
   > 2,8 мб * 14930 RPS = 334,432 Гбит/с

   Получение: *88,12 Гбит/с*
   > 2,8 мб * 0,086 * 45741 RPS = 88,12 Гбит/с
5. Работа с вложениями в сообщениях:

   Загрузка: *54,88 Гбит/с*
   > 2 мб * 3430 RPS = 54,88 Гбит/с

   Получение: *137,216 Гбит/с*
   > 2 мб * 8576 RPS = 137,216 Гбит/с

## Логическая схема базы данных
![схема бд](imgs/db_schmeme.png)
