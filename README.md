...
2. Голосовые сообщения: *473 611,1 Тб*
   ...
   > Принимаем среднюю длину голосового сообщения за 15 секунд, кодирование OGG (192 кбит / с), откуда вес одного ГС 2880 кбит

   > Итого 1412,6 млрд * 2880 кбит = 473 611,1 Тб
3. Вложения в сообщениях: *572 662,4 Тб *
   ...
   > Тогда объем хранилища вложений составит 15012,5  млрд * 2% * 2мб = 600,48 млрд мб = 572 662,4 Тб 
#### Сетевой трафик
...
4. Работа с голосовыми сообщениями: 
   ...
   Создание: *43 Гбит/с*
   > 2880 кбит * 14930 RPS = 43 Гбит/с

   Получение: *11,33 Гбит/с*
   > 2880 кбит * 0,086 * 45741 RPS = 11,33 Гбит/с
...
#### Ускорение работы. Индексы

Таблица пользователей:
- По `first_name` + `last_name`

Таблица сообщений:
- По `create_date` + `chat_id`
...
Таблица сообщений: 
- по `chat_id`
  > т.к. каждое сообщение привязано к соттветствующему чату

### Redis
Хранение сессий пользователей

> Redis - *in-memory* база данных, следовательно доступ к записям осуществляется со скоростью соизмеримой с ОЗУ.Также, Redis имеет возможность master-slave репликацию и встроенное API для работы с Memcached.
...
| Redis                 | Backend              | in-memory БД для хранения сессий пользователей. Обладает очень малым времени обработки запроса на чтение и запись, популярна, большое комьюнити. Шардирование.                                       |
